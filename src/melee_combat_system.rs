...
impl<'a> System<'a> for MeleeCombatSystem {
    ...
    fn run(&mut self, SystemData) {
        let (entities, mut wants_melee, name, combat_stats, mut suffer_damage) = data;

        for(_entity, wants_melee, name, stats) in (&entities, &wants_melee, &name, &combat_stats).join() {
            if stats.hp > 0 {
